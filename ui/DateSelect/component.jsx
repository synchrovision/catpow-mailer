Catpow.UI.DateSelect = (props) => {
		...
		switch (action.type) {
			...
			case "update": {
				const d = action.value
					...
					: new Date(action.year || state.year || now.getFullYear(), (action.month || state.month || now.getMonth() + 1) - 1, action.date || state.date || now.getDate());
				if (isNaN(d.getTime())) {
	...
	return (
		<div className={className}>
			...
			{state.value && <Catpow.Components.HiddenValues name={props.name} value={state.value} />}
		</div>
	);
};
