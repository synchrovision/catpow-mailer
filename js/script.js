...
Catpow.MailFormHostUrl = window.location.toString();
Catpow.MailForm = function (form) {
	...
	cmf.send = function (data, cb = false) {
		...
		xhr.onload = function () {
			if (xhr.readyState === 4 && xhr.status === 200) {
				...
				if (res.uri) {
					const destUrl = res.uri === "#" ? new URL(Catpow.MailFormHostUrl) : new URL(res.uri, Catpow.MailFormHostUrl);
					if (destUrl.pathname !== window.location.pathname) {
						history.pushState(res, null, destUrl.toString());
						if (window.gtag) {
							gtag("set", "page_path", destUrl.toString());
							gtag("event", "page_view");
						}
						if (window.ga) {
							ga("send", "pageview", destUrl.toString());
						}
					}
				}
				if (cb) {
	...
	cmf.showError = function (error) {
		Object.keys(error).map(function (key) {
			if (key === "@form") {
				...
				cmf.inputs[key].alert(error[key]);
			}
		});
	};
	...
	cmf.reset = function () {
		...
		Array.prototype.forEach.call(form.querySelectorAll(".cmf-button"), function (button) {
			if (button.classList.contains("sealed")) {
				cmf.sealedButtons.push(button);
			}
			button.addEventListener("click", function () {
				if (cmf.isLocked || button.classList.contains("disabled")) {
					return;
				}
	...
	cmf.updateState = function () {
		cmf.agreed = cmf.agreementCheckboxes.every(function (checkbox) {
	...
	new Promise(function (resolve) {
		...
		new Promise(function (resolve) {
			if (Catpow.MailForm.deps) {
				Catpow.MailForm.deps.styles.forEach(function (style) {
					Catpow.MailForm.loadStyle(style);
				});
				if (Catpow.MailForm.deps.scripts.length) {
					Promise.all(
						Catpow.MailForm.deps.scripts.map(function (script) {
